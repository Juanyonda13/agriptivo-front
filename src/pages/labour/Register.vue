          ...
          <v-form ref='form' v-model='validForm'>
            <v-row>
              <v-col cols='12' sm='6' md='12'>
                <v-text-field v-model='name_labour' label='Nombre de tu Actividad*' :rules='processRules' maxlength='50'
            ...
            <v-row>
              <v-col cols='12' sm='6' md='12'>
                <v-text-field v-model='cost_labour' label='Costo*' :rules='processRules' counter variant='outlined'
                  type='number'></v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col cols='12' sm='6' md='12'>
                <v-text-field v-model='working_hours_labour' label='Horas de trabajo*' :rules='processRules' counter
                  variant='outlined' type='number'></v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="12" sm="6" md="12">
                <v-autocomplete label="Trabajadores*" clearable item-title="name_user" item-value="id_user"
                  :items="jornareros" variant="outlined" v-model="id_user"></v-autocomplete>
              </v-col>
            </v-row>
          </v-form>
        </v-container>
...
const dataReset = () => {
  ...
  id_user.value = null
  cost_labour.value = null
  working_hours_labour.value = null
}
